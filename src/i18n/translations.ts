export type Lang = 'de' | 'en';

export const translations = {
  de: {
    nav: {
      services: 'Services',
      fleet: 'Fahrzeugflotte',
      gallery: 'Galerie',
      booking: 'Buchung',
    },
    header: {
      menuOpen: 'Menü öffnen',
      menuClose: 'Menü schließen',
      callLabel: 'Jetzt anrufen: +49 177 2472408',
      whatsapp: 'WhatsApp Chat',
      home: 'VIPSHUTTLE24 – Startseite',
    },
    hero: {
      badge: 'Premium Chauffeur Service Düsseldorf',
      headline1: 'Ihre Reise beginnt',
      headline2: 'mit Exzellenz',
      description:
        'Erstklassiger Chauffeur-Service in Düsseldorf und NRW. Diskretion, Pünktlichkeit und Komfort auf höchstem Niveau.',
      cta1: 'Jetzt buchen',
      cta1Label: 'Jetzt Chauffeur buchen',
      cta2: 'Services entdecken',
      cta2Label: 'Unsere Chauffeur-Services entdecken',
      stat1Value: '24/7',
      stat1Label: 'Verfügbar',
      stat2Value: '20+',
      stat2Label: 'Jahre Erfahrung',
      stat3Value: '100%',
      stat3Label: 'Diskretion',
      statsAriaLabel: 'Unsere Versprechen',
    },
    services: {
      sectionLabel: 'Unsere Services',
      badge: 'Unsere Services',
      headline: 'Exzellenz in jedem Detail',
      subline:
        'Von Flughafentransfers bis zu maßgeschneiderten Gruppenreisen – erstklassige Mobilität für jeden Anlass.',
      cta: 'Jetzt anfragen',
      bottomText: 'Benötigen Sie einen individuellen Service?',
      bottomCta: 'Persönliche Beratung anfragen',
      items: [
        {
          title: 'VIP & Chauffeur Service',
          subtitle: 'Diskretion auf höchstem Niveau',
          description:
            'Regelmäßige Betreuung von Geschäftsleuten, Delegationen und VIP-Gästen. Mehrsprachige, ortskundige Fahrer sorgen für 24/7 erstklassigen Service – diskret, zuverlässig und professionell.',
          features: ['VIP-Gästebetreuung', 'Mehrsprachige Fahrer', 'Diskrete Begleitung', '24/7 Premium Service'],
        },
        {
          title: 'Airport Transfer',
          subtitle: 'Pünktlich und stressfrei',
          description:
            'Professioneller Flughafentransfer zu allen deutschen und internationalen Flughäfen. Mit Flugüberwachung in Echtzeit passen wir uns flexibel an Ihre Ankunftszeit an.',
          features: ['Meet & Greet Service', 'Gepäckassistenz', 'Wartezeit inkludiert', 'Alle Flughäfen in NRW'],
        },
        {
          title: 'Business & Events',
          subtitle: 'Geschäftsreisen und Veranstaltungen',
          description:
            'Professionelle Organisation Ihrer Geschäftsreisen, Roadshows und Messe-Transfers in ganz NRW. Ortskundige Fahrer, VIP-Gästebetreuung und Koordination für Galas, Konzerte und Sportevents.',
          features: ['Geschäftsfahrten & Roadshows', 'Messe- & Eventshuttle', 'VIP- und Gästebetreuung', 'Galas, Konzerte, Sportevents'],
        },

        {
          title: 'Private Anlässe',
          subtitle: 'Jubiläen, Geburtstage & mehr',
          description:
            'Unvergessliche Fahrten für Ihre persönlichen Höhepunkte – Jubiläen, Geburtstage, Abiball, Stadttouren und Sightseeing. Stilvolle Fahrzeuge und aufmerksame Chauffeure für jeden besonderen Moment.',
          features: ['Jubiläen & Geburtstage', 'Abiball & Schulabschluss', 'Stadttouren / Sightseeing', 'Flexible Zeiteinteilung'],
        },
        {
          title: 'Hotel & Messe Transfer',
          subtitle: 'Komfortabel von A nach B',
          description:
            'Zuverlässige Transfers zwischen Hotel, Messe und Flughafen. Ob Expo Real, Bauma oder IAA – wir bringen Sie pünktlich, sicher und komfortabel ans Ziel und wieder zurück.',
          features: ['Hoteltransfer', 'Messefahrten', 'Door-to-Door-Service', 'Gepäckassistenz'],
        },
                {
          title: 'Gruppen & Bus Transfer',
          subtitle: 'Für große Gruppen und Events',
          description:
            'Komfortabler Gruppentransfer mit Mercedes-Benz Sprinter oder Tourismo-Reisebus für bis zu 50 Personen. Ideal für Firmenevents, Messefahrten und Gruppenreisen in ganz NRW.',
          features: ['Bis zu 50 Personen', 'Mercedes Sprinter & Tourismo', 'Mehrtägige Touren möglich', 'Messe- & Eventshuttle'],
        },
      ],
    },
    fleet: {
      sectionLabel: 'Fahrzeugflotte',
      badge: 'Fahrzeugflotte',
      headline: 'Exklusive Mercedes-Flotte für jeden Anlass',
      subline:
        'Ausschließlich gepflegte Mercedes-Fahrzeuge – Limousinen, Vans und Reisebusse für höchste Ansprüche an Komfort und Stil.',
      viewLabel: 'ansehen',
      closeLabel: 'Schließen',
      prevLabel: 'Vorheriges Fahrzeug',
      nextLabel: 'Nächstes Fahrzeug',
      stat1Value: '100%',
      stat1Label: 'Zuverlässig',
      stat2Value: '100%',
      stat2Label: 'Diskretion',
      stat3Value: '100%',
      stat3Label: 'Professionell',
      vehicles: [
        {
          name: 'Mercedes-Benz S-Klasse',
          category: 'Luxus-Limousine',
          description:
            'Unsere Flaggschiff-Limousine für höchste Ansprüche. Maximaler Komfort, modernste Technologie und überlegene Eleganz.',
        },
        {
          name: 'Mercedes-Benz E-Klasse',
          category: 'Business-Limousine',
          description: 'Eleganz und Komfort vereint. Ideal für Geschäftsreisen und repräsentative Anlässe.',
        },
        {
          name: 'Mercedes-Benz Sprinter',
          category: 'VIP-Van',
          description: 'Großzügiger Raum für bis zu 7 Personen ohne Abstriche beim Komfort.',
        },
        {
          name: 'Mercedes-Benz V-Klasse',
          category: 'Airport Flotte',
          description: 'V-Klasse – perfekte Koordination für Gruppen-Transfers am Flughafen.',
        },
        {
          name: 'Mercedes-Benz Tourismo',
          category: 'Reisebus (schwarz)',
          description: 'Unser schwarzer Tourismo für Gruppen bis zu 50 Personen. Klimatisiert, premium ausgestattet.',
        },
        {
          name: 'Mercedes-Benz Sprinter',
          category: 'Sprinter',
          description: 'Modernster Sprinter in eleganter Blau-Grau-Lackierung für unvergessliche Gruppenreisen.',
        },
      ],
    },
    gallery: {
      sectionLabel: 'Bildergalerie VipShuttle24',
      badge: 'Galerie',
      headline1: 'Erleben Sie',
      headline2: 'unseren Service',
      subline: 'Einblicke in unsere Fahrzeuge und unvergessliche Momente mit VIPSHUTTLE24 in Düsseldorf und NRW.',
      showMore: (n: number) => `Alle ${n} weiteren Fotos anzeigen`,
      showLess: 'Weniger anzeigen',
      closeLabel: 'Schließen',
      prevLabel: 'Vorheriges Bild',
      nextLabel: 'Nächstes Bild',
      openLabel: 'Bild öffnen',
      counter: (cur: number, total: number) => `${cur} / ${total}`,
      playLabel: 'Video abspielen / pausieren',
      muteLabel: 'Ton an/aus',
    },
    booking: {
      sectionLabel: 'Buchungsanfrage',
      badge: 'Buchungsanfrage',
      headline: 'Ihre Fahrt beginnt hier',
      subline: 'Stellen Sie eine unverbindliche Anfrage. Wir melden uns innerhalb von 2 Stunden bei Ihnen zurück.',
      contact: {
        title: 'Kontaktinformationen',
        phone: 'Telefon',
        phoneAvail: '24/7 verfügbar',
        whatsapp: 'WhatsApp',
        whatsappSub: 'Schnellste Antwort',
        whatsappDirect: 'Direkter Chat',
        email: 'E-Mail',
        emailSub: 'Per E-Mail erreichbar',
        address: 'Hauptsitz',
        mapLink: 'Route planen →',
        availTitle: 'Verfügbarkeit',
        availText: 'Unser Service ist rund um die Uhr verfügbar.',
        availEmphasis: '7 Tage die Woche, 24 Stunden am Tag',
      },
      why: {
        title: 'Warum VIPSHUTTLE24?',
        items: [
          { title: 'Festpreisgarantie', text: 'Kein Taxameter – Sie kennen Ihren Preis vor der Fahrt.' },
          { title: 'Diskret & pünktlich', text: 'Absolute Verschwiegenheit und minutengenaue Zuverlässigkeit.' },
          { title: 'Nur Premium-Fahrzeuge', text: 'Ausschließlich gepflegte Mercedes-Benz – vom Sedan bis zum Reisebus.' },
          { title: 'Kurzfristig buchbar', text: 'Schnelle Verfügbarkeit – rufen Sie uns direkt an.' },
        ],
      },
      form: {
        name: 'Ihr Name',
        namePlaceholder: 'Max Mustermann',
        email: 'E-Mail',
        emailPlaceholder: 'ihre@email.de',
        phone: 'Telefon',
        phonePlaceholder: '+49 ...',
        service: 'Gewünschter Service',
        servicePlaceholder: 'Bitte wählen',
        serviceOptions: {
          sprinter: 'Mercedes-Benz Sprinter',
          airport: 'Airport Transfer',
          business: 'Business & Events',
          vip: 'VIP & Chauffeur Service',
          group: 'Gruppen & Bus Transfer',
          private: 'Private Anlässe',
          hotel: 'Hotel & Messe Transfer',
          hourly: 'Stundenweise Buchung',
          other: 'Sonstiges',
        },
        date: 'Datum',
        time: 'Uhrzeit',
        passengers: 'Anzahl Personen',
        passengerPlaceholder: 'Bitte wählen',
        person: 'Person',
        persons: 'Personen',
        pickup: 'Abholort',
        pickupPlaceholder: 'z.B. Düsseldorf Flughafen, Terminal A',
        destination: 'Zielort',
        destinationPlaceholder: 'z.B. Innenstadt Düsseldorf',
        message: 'Weitere Wünsche',
        messagePlaceholder: 'Kindersitz, Rollstuhlzugang, besondere Wünsche...',
        required:
          '* Pflichtfelder. Durch das Absenden öffnet sich Ihr E-Mail-Programm mit Ihrer vorausgefüllten Anfrage.',
        submit: 'Anfrage per E-Mail senden',
      },
      emailSubject: (service: string, date: string) => `Buchungsanfrage – ${service} – ${date || 'Datum offen'}`,
      emailBody: (d: Record<string, string>) =>
        `Sehr geehrtes VipShuttle24-Team,\n\nhiermit stelle ich eine unverbindliche Buchungsanfrage:\n\n──────────────────────────────\nPERSÖNLICHE DATEN\n──────────────────────────────\nName:         ${d.name}\nE-Mail:       ${d.email}\nTelefon:      ${d.phone}\n\n──────────────────────────────\nFAHRTDETAILS\n──────────────────────────────\nService:      ${d.service}\nDatum:        ${d.date}\nUhrzeit:      ${d.time}\nPersonen:     ${d.passengers}\nAbholort:     ${d.pickup}\nZielort:      ${d.destination}\n\n──────────────────────────────\nWEITERE INFORMATIONEN\n──────────────────────────────\n${d.message}\n──────────────────────────────\n\nBitte nehmen Sie schnellstmöglich Kontakt mit mir auf.\n\nMit freundlichen Grüßen,\n${d.name}`,
    },
    footer: {
      tagline: 'Ihr Premium Chauffeur-Service in Düsseldorf und NRW. Sicherheit, Komfort und Eleganz in jedem Detail.',
      navTitle: 'Navigation',
      servicesTitle: 'Services',
      contactTitle: 'Kontakt',
      copyright: (year: number) => `© ${year} VIPSHUTTLE24. Alle Rechte vorbehalten.`,
      impressum: 'Impressum',
      datenschutz: 'Datenschutz',
      agb: 'AGB',
      whatsappLabel: 'WhatsApp Chat öffnen',
      fbLabel: 'VipShuttle24 auf Facebook',
      igLabel: 'VipShuttle24 auf Instagram',
      waLabel: 'VipShuttle24 auf WhatsApp kontaktieren',
      phoneLabel: 'Telefon: +49 177 2472408',
      emailLabel: 'E-Mail: info@vipshuttle24.de',
      addressLabel: 'Adresse: Schlesische Str. 104, 40231 Düsseldorf',
      hotline: '24/7 Hotline',
      navItems: [
        { label: 'Services', href: '#services' },
        { label: 'Fahrzeugflotte', href: '#fleet' },
        { label: 'Galerie', href: '#gallery' },
        { label: 'Buchung', href: '#booking' },
      ],
      servicesList: [
        'Airport Transfer Düsseldorf',
        'Hochzeitsfahrten NRW',
        'Corporate Roadshows',
        'Stundenweise Buchung',
        'VIP-Service',
        'Gruppen & Bus Transfer',
      ],
    },
    legal: {
      back: '← Zurück zur Website',
      impressum: {
        pageTitle: 'Impressum | VIPSHUTTLE24 Düsseldorf',
        h1: 'Impressum',
        s1Title: 'Angaben gemäß § 5 TMG',
        s1Name: 'Ahmet Bostanci',
        s1Company: 'Vipshuttle24',
        s1Street: 'Schlesische Str. 104',
        s1City: '40231 Düsseldorf',
        s2Title: 'Kontakt',
        s2Mobile: 'Mobil',
        s2Phone: 'Telefon',
        s2Email: 'E-Mail',
        s3Title: 'Allgemeiner Disclaimer',
        s3Text:
          'Für Vollständigkeit, Fehler redaktioneller und technischer Art, Auslassungen sowie die Richtigkeit der Eintragungen kann keine Haftung übernommen werden. Insbesondere kann keine Gewähr für die Vollständigkeit und Richtigkeit von Informationen übernommen werden, die über weiterführende Links erreicht werden. Anbieter sind für die eigenen Inhalte, die sie zur Nutzung bereithalten, nach den allgemeinen Gesetzen verantwortlich. Die Verweise durch Hyperlinks auf Inhalte fremder Webseiten dienen lediglich der Information.',
        s4Title: 'Streitschlichtung',
        s4Text: 'Die Europäische Kommission stellt eine Plattform zur Online-Streitbeilegung (OS) bereit:',
        s4Suffix:
          'Wir sind nicht bereit oder verpflichtet, an Streitbeilegungsverfahren vor einer Verbraucherschlichtungsstelle teilzunehmen.',
      },
      datenschutz: {
        pageTitle: 'Datenschutzerklärung | VIPSHUTTLE24 Düsseldorf',
        h1: 'Datenschutzerklärung',
        s1Title: '1. Datenschutz auf einen Blick',
        s1h2: 'Allgemeine Hinweise',
        s1p1:
          'Die folgenden Hinweise geben einen einfachen Überblick darüber, was mit Ihren personenbezogenen Daten passiert, wenn Sie diese Website besuchen. Personenbezogene Daten sind alle Daten, mit denen Sie persönlich identifiziert werden können.',
        s1h3: 'Datenerfassung auf dieser Website',
        s1p2:
          'Die Datenverarbeitung auf dieser Website erfolgt durch den Websitebetreiber. Ihre Daten werden zum einen dadurch erhoben, dass Sie uns diese mitteilen (z. B. Kontaktformular). Andere Daten werden automatisch beim Besuch der Website durch unsere IT-Systeme erfasst (technische Daten wie Internetbrowser, Betriebssystem oder Uhrzeit des Seitenaufrufs).',
        s1h4: 'Ihre Rechte',
        s1p3:
          'Sie haben jederzeit das Recht auf unentgeltliche Auskunft über Herkunft, Empfänger und Zweck Ihrer gespeicherten personenbezogenen Daten sowie ein Recht auf Berichtigung, Sperrung oder Löschung dieser Daten. Des Weiteren steht Ihnen ein Beschwerderecht bei der zuständigen Aufsichtsbehörde zu.',
        s2Title: '2. Hosting',
        s2h2: 'RAIDBOXES',
        s2p1:
          'Wir hosten die Inhalte unserer Website bei RAIDBOXES GmbH, Hafenstr. 32, 48151 Münster. Wenn Sie unsere Website besuchen, erfasst RAIDBOXES verschiedene Logfiles inklusive Ihrer IP-Adressen. Die Verwendung von RAIDBOXES erfolgt auf Grundlage von Art. 6 Abs. 1 lit. f DSGVO.',
        s2h3: 'Cloudflare',
        s2p2:
          'Wir nutzen den Service von Cloudflare Inc., 101 Townsend St., San Francisco, CA 94107, USA. Cloudflare bietet ein weltweit verteiltes Content Delivery Network. Der Einsatz erfolgt auf Grundlage von Art. 6 Abs. 1 lit. f DSGVO. Das Unternehmen verfügt über eine Zertifizierung nach dem EU-US Data Privacy Framework (DPF).',
        s3Title: '3. Verantwortliche Stelle',
        s3Name: 'Vipshuttle24',
        s3Person: 'Ahmet Bostanci',
        s3Street: 'Schlesische Str. 104',
        s3City: '40231 Düsseldorf',
        s3Phone: 'Telefon',
        s3Mobile: 'Mobil',
        s3Email: 'E-Mail',
        s4Title: '4. Datenerfassung auf dieser Website',
        s4h2: 'Cookies',
        s4p1:
          'Unsere Internetseiten verwenden sogenannte Cookies. Cookies sind kleine Datenpakete, die auf Ihrem Endgerät gespeichert werden. Sie können Ihren Browser so einstellen, dass Sie über das Setzen von Cookies informiert werden.',
        s4h3: 'Kontaktformular & E-Mail',
        s4p2:
          'Wenn Sie uns per Kontaktformular oder E-Mail Anfragen zukommen lassen, werden Ihre Angaben inklusive der von Ihnen angegebenen Kontaktdaten zwecks Bearbeitung der Anfrage bei uns gespeichert. Diese Daten geben wir nicht ohne Ihre Einwilligung weiter. Rechtsgrundlage: Art. 6 Abs. 1 lit. b DSGVO.',
        s4h4: 'Kommunikation via WhatsApp',
        s4p3:
          'Für die Kommunikation nutzen wir WhatsApp Business der WhatsApp Ireland Limited, 4 Grand Canal Square, Dublin 2, Irland. Die Kommunikation erfolgt über Ende-zu-Ende-Verschlüsselung. Rechtsgrundlage: Art. 6 Abs. 1 lit. f DSGVO. Das Unternehmen verfügt über eine Zertifizierung nach dem EU-US Data Privacy Framework.',
        s5Title: '5. Soziale Medien',
        s5p1:
          'Auf dieser Website sind Elemente von sozialen Medien (Facebook, Instagram) eingebunden. Diese werden nur zusammen mit der Shariff-Lösung eingesetzt, die verhindert, dass beim Seitenaufruf bereits Daten übertragen werden. Erst bei aktivem Klick auf das Social-Media-Element wird eine Verbindung hergestellt (Art. 6 Abs. 1 lit. a DSGVO).',
        s6Title: '6. Analyse-Tools',
        s6h2: 'WP Statistics',
        s6p1:
          'Diese Website nutzt WP Statistics (Veronalabs, Tallinn, Estland) zur anonymisierten Auswertung der Besucherzugriffe. Die erfassten Daten werden ausschließlich auf unserem eigenen Server gespeichert. IP-Adressen werden anonymisiert. Rechtsgrundlage: Art. 6 Abs. 1 lit. f DSGVO.',
        s7Title: '7. Plugins und Tools',
        s7h2: 'YouTube (erweiterter Datenschutz)',
        s7p1:
          'Diese Website bindet Videos von YouTube ein (Google Ireland Limited, Dublin 4, Irland). Wir nutzen den erweiterten Datenschutzmodus. Rechtsgrundlage: Art. 6 Abs. 1 lit. f DSGVO. Google ist nach dem EU-US Data Privacy Framework zertifiziert.',
        s7h3: 'Font Awesome',
        s7p2:
          'Diese Seite nutzt Font Awesome (Fonticons, Inc., Cambridge, Massachusetts, USA) zur Darstellung von Icons. Rechtsgrundlage: Art. 6 Abs. 1 lit. f DSGVO.',
        s7h4: 'OpenStreetMap',
        s7p3:
          'Wir binden OpenStreetMap auf unserem eigenen Server ein, sodass keine Verbindung zu Servern Dritter erfolgt. Rechtsgrundlage: Art. 6 Abs. 1 lit. f DSGVO.',
        s8Title: '8. SSL-Verschlüsselung',
        s8p1:
          'Diese Seite nutzt aus Sicherheitsgründen eine SSL- bzw. TLS-Verschlüsselung. Eine verschlüsselte Verbindung erkennen Sie daran, dass die Adresszeile des Browsers von http:// auf https:// wechselt.',
      },
      agb: {
        pageTitle: 'AGB | VIPSHUTTLE24 Düsseldorf',
        h1: 'Allgemeine Geschäftsbedingungen',
        preise: { title: 'Preise', text: 'Alle Preise sind in Euro und inklusive der gesetzlichen Mehrwertsteuer in Höhe von 19 %.' },
        zahlung: { title: 'Zahlung', text: 'Zahlung für Dienstleistungen ist per Banküberweisung im Voraus oder mit Kreditkarte möglich. Wir akzeptieren keine Barzahlungen.' },
        leistung: { title: 'Leistungsumfang', text: 'Mietpreise beinhalten Fahrdienstleistungen, Treibstoffkosten und kostenloses Wasser im Fahrzeug. Park- und Mautgebühren werden extra berechnet.' },
        mindest: { title: 'Mindestmietdauer', text: 'Die Mindestmietdauer für Stundenbuchungen beträgt drei Stunden.' },
        wartezeit: { title: 'Wartezeit', text: '30 Minuten Wartezeit am Flughafen und 15 Minuten an anderen Abholorten sind im Preis enthalten. Weitere Wartezeit am Abholort wird im 30-Minuten-Takt berechnet.' },
        fahrpreis: { title: 'Fahrpreis', text: 'Der Fahrpreis ändert sich nicht während der Fahrt aufgrund von verkehrsbedingten Änderungen der Fahrstrecke oder Wartezeiten.' },
        rauchen: { title: 'Rauchverbot', text: 'Das Rauchen ist in jedem unserer Fahrzeuge verboten.' },
        haftungKunde: { title: 'Haftung des Kunden', text: 'Die Kunden übernehmen die volle finanzielle Haftung für Schäden, die durch Sie oder Mitglieder ihrer Vertragspartei während der Miet-/Fahrzeit verursacht wurden.' },
        haftungsausschluss: { title: 'Haftungsausschluss', text: 'Das Unternehmen ist nicht verantwortlich für Verzögerungen wegen Wetter, Verkehr oder unsicherer Straßenverhältnisse (Naturereignisse, Unfälle oder unvorhersehbare technische Pannen). In diesen Fällen darf vom Vertrag ohne Ersatzleistung zurückgetreten werden. Unsere Haftung ist grundsätzlich auf die gesetzlich vorgeschriebene Betriebshaftpflichtversicherung beschränkt.' },
        storno: {
          title: 'Stornierungsbedingungen',
          row1label: 'Stornierung bis 7 Tage vor Abreise',
          row1value: 'Kostenlos',
          row2label: 'Stornierung bis 2 Tage vor Abreise',
          row2value: '50 %',
          row3label: 'Stornierung bis 24 Stunden vor Abreise',
          row3value: '75 %',
          row4label: 'Stornierung ab 24 Stunden vor Abreise',
          row4value: '100 %',
        },
        gericht: { title: 'Gerichtsstand', text: 'Gerichtsstand ist ausschließlich Düsseldorf, soweit der Auftraggeber Kaufmann, juristische Person des öffentlichen Rechts oder ein öffentlich-rechtliches Sondervermögen ist. Dies gilt gleichermaßen, wenn der Auftraggeber keinen allgemeinen Gerichtsstand in Deutschland oder einem anderen Mitgliedsstaat der EU hat.' },
      },
      links: {
        impressum: 'Impressum',
        datenschutz: 'Datenschutz',
        agb: 'AGB',
        home: 'Startseite',
      },
    },
  },

  en: {
    nav: {
      services: 'Services',
      fleet: 'Fleet',
      gallery: 'Gallery',
      booking: 'Booking',
    },
    header: {
      menuOpen: 'Open menu',
      menuClose: 'Close menu',
      callLabel: 'Call us: +49 177 2472408',
      whatsapp: 'WhatsApp Chat',
      home: 'VIPSHUTTLE24 – Home',
    },
    hero: {
      badge: 'VIP Chauffeur Service Germany – English Speaking',
      headline1: "Germany's Premier",
      headline2: 'VIP Chauffeur Service',
      description:
        'Luxury chauffeur service across Germany for celebrities, executives & VIP clients. English-speaking, discreet drivers. Exclusive Mercedes fleet. 24/7 availability from Düsseldorf to all of Germany and Europe.',
      cta1: 'Book Your Ride',
      cta1Label: 'Book a VIP chauffeur in Germany',
      cta2: 'Our Services',
      cta2Label: 'Explore our VIP chauffeur services in Germany',
      stat1Value: '24/7',
      stat1Label: 'Available',
      stat2Value: '20+',
      stat2Label: 'Years Experience',
      stat3Value: '100%',
      stat3Label: 'Discreet',
      statsAriaLabel: 'Our service standards',
    },
    services: {
      sectionLabel: 'VIP Chauffeur Services in Germany',
      badge: 'Our Services',
      headline: 'VIP Chauffeur & Luxury Transport Services Across Germany',
      subline:
        'From VIP airport pickups to full-day celebrity chauffeur service — professional Mercedes-Benz transportation for international clients across Germany and Europe.',
      cta: 'Request a Quote',
      bottomText: 'Need a custom itinerary or multi-city tour?',
      bottomCta: 'Contact Us Directly',
      items: [
        {
          title: 'VIP & Celebrity Chauffeur Service',
          subtitle: 'Private driver for celebrities, executives & VIPs in Germany',
          description:
            'Dedicated chauffeur service for A-list celebrities, C-suite executives, athletes, politicians, and high-profile international clients. Our English-speaking drivers are trained in discretion, security awareness, and professional etiquette. Available 24/7 anywhere in Germany.',
          features: ['Celebrity & athlete transport', 'English-speaking chauffeurs', 'Full discretion guaranteed', '24/7 on-call availability'],
        },
        {
          title: 'VIP Airport Transfer Germany',
          subtitle: 'Meet & greet service at all German airports',
          description:
            'First-class airport transfers to and from Düsseldorf (DUS), Frankfurt (FRA), Munich (MUC), Hamburg (HAM), Berlin (BER) and Cologne/Bonn (CGN). Real-time flight monitoring means your driver is always there when you land — even if your flight is delayed.',
          features: ['All major German airports', 'Real-time flight tracking', 'Meet & greet with name board', 'Luggage assistance included'],
        },
        {
          title: 'Corporate Travel & Roadshow Chauffeur',
          subtitle: 'Executive transportation for business delegations in Germany',
          description:
            'End-to-end logistics for corporate roadshows, investor tours, trade fair transfers, and delegation management across Germany and Europe. We cover Messe Düsseldorf, Hannover Messe, IAA Frankfurt, and all major German trade shows with dedicated vehicles and a personal coordinator.',
          features: ['Corporate roadshow logistics', 'Trade fair shuttle (DUS, FRA, MUC)', 'Delegation management', 'Multi-vehicle fleet available'],
        },
        {
          title: 'Private Hire & Special Occasions',
          subtitle: 'Luxury car hire for personal events in Germany',
          description:
            "Make any occasion unforgettable with a private Mercedes chauffeur. Whether it's a birthday dinner in Düsseldorf, a Rhine Valley tour, city sightseeing, or a gala event — our drivers take care of every detail so you can enjoy the moment.",
          features: ['Sightseeing & city tours', 'Special event transportation', 'Flexible hourly booking', 'Complimentary water & amenities'],
        },
        {
          title: 'Hotel & Conference Transfer',
          subtitle: 'Seamless transfers between hotels, venues & airports',
          description:
            'Reliable point-to-point transfers between luxury hotels, conference centers, trade fair halls, and airports across Germany. Fixed prices, no hidden fees, and a professionally dressed chauffeur waiting when you arrive.',
          features: ['Hotel-to-airport transfers', 'Conference & expo shuttles', 'Door-to-door service', 'Fixed-price guarantee'],
        },
        {
          title: 'Group & Coach Charter Germany',
          subtitle: 'Luxury group transportation for up to 50 passengers',
          description:
            'Large-group transportation across Germany in a Mercedes-Benz Tourismo coach or Sprinter fleet. Perfect for corporate groups, sports teams, concert tours, and private travel parties. Available for single trips or multi-day Germany tours.',
          features: ['Up to 50 passengers', 'Mercedes-Benz Tourismo coach', 'Multi-day Germany tours', 'Sports teams & concert tours'],
        },
      ],
    },
    fleet: {
      sectionLabel: 'Luxury Mercedes Fleet – VIP Chauffeur Germany',
      badge: 'Our Fleet',
      headline: 'Exclusive Mercedes-Benz Fleet for VIP Travel in Germany',
      subline:
        'Every vehicle in our fleet is a Mercedes-Benz — S-Class, E-Class, V-Class, Sprinter, or Tourismo coach. All cars are no more than 3 years old, fully insured, and maintained to the highest standards.',
      viewLabel: 'view',
      closeLabel: 'Close',
      prevLabel: 'Previous vehicle',
      nextLabel: 'Next vehicle',
      stat1Value: '100%',
      stat1Label: 'Reliable',
      stat2Value: '100%',
      stat2Label: 'Discreet',
      stat3Value: '100%',
      stat3Label: 'Professional',
      vehicles: [
        {
          name: 'Mercedes-Benz S-Class',
          category: 'Flagship Luxury Sedan',
          description:
            'The pinnacle of automotive luxury. Our S-Class is the vehicle of choice for celebrities, heads of state, and senior executives who demand the very best.',
        },
        {
          name: 'Mercedes-Benz E-Class',
          category: 'Executive Business Sedan',
          description:
            'The ideal business travel companion — elegant, refined, and comfortable for long-distance executive transfers across Germany.',
        },
        {
          name: 'Mercedes-Benz Sprinter',
          category: 'VIP People Carrier',
          description:
            'Premium VIP people carrier for up to 7 passengers. Ideal for team travel, delegations, and groups requiring extra luggage capacity.',
        },
        {
          name: 'Mercedes-Benz V-Class',
          category: 'Airport VIP Van',
          description:
            'The V-Class is our premier airport transfer vehicle — spacious, smooth, and perfect for coordinated group arrivals.',
        },
        {
          name: 'Mercedes-Benz Tourismo',
          category: 'Luxury Coach (Black)',
          description:
            'Our all-black Tourismo coach for groups up to 50. Fully air-conditioned, premium seating — ideal for concert tours, corporate groups, and long-distance Germany tours.',
        },
        {
          name: 'Mercedes-Benz Sprinter',
          category: 'Premium Sprinter Van',
          description:
            'Our distinctive blue-grey Sprinter for unforgettable group journeys — combining style, capacity and comfort.',
        },
      ],
    },
    gallery: {
      sectionLabel: 'VipShuttle24 Photo Gallery',
      badge: 'Gallery',
      headline1: 'Experience',
      headline2: 'our service',
      subline: 'Insights into our vehicles and unforgettable moments with VIPSHUTTLE24 in Düsseldorf and NRW.',
      showMore: (n: number) => `Show all ${n} more photos`,
      showLess: 'Show less',
      closeLabel: 'Close',
      prevLabel: 'Previous image',
      nextLabel: 'Next image',
      openLabel: 'Open image',
      counter: (cur: number, total: number) => `${cur} / ${total}`,
      playLabel: 'Play / pause video',
      muteLabel: 'Toggle mute',
    },
    booking: {
      sectionLabel: 'Booking Request',
      badge: 'Booking Request',
      headline: 'Your ride starts here',
      subline: 'Submit a non-binding request. We will get back to you within 2 hours.',
      contact: {
        title: 'Contact Information',
        phone: 'Phone',
        phoneAvail: 'Available 24/7',
        whatsapp: 'WhatsApp',
        whatsappSub: 'Fastest response',
        whatsappDirect: 'Direct chat',
        email: 'Email',
        emailSub: 'Available by email',
        address: 'Head office',
        mapLink: 'Get directions →',
        availTitle: 'Availability',
        availText: 'Our service is available around the clock.',
        availEmphasis: '7 days a week, 24 hours a day',
      },
      why: {
        title: 'Why Choose VIPSHUTTLE24?',
        items: [
          { title: 'Fixed-Price Guarantee', text: 'No meters, no surprises. Your price is agreed before we drive — full stop.' },
          { title: 'English-Speaking Drivers', text: 'All our chauffeurs communicate fluently in English. International clients are our specialty.' },
          { title: 'Exclusive Mercedes Fleet', text: 'S-Class, E-Class, V-Class, Sprinter — every vehicle is a recent-model Mercedes-Benz.' },
          { title: 'Absolute Discretion', text: 'We regularly transport celebrities, politicians, and executives. Confidentiality is part of our DNA.' },
        ],
      },
      form: {
        name: 'Your name',
        namePlaceholder: 'John Doe',
        email: 'Email',
        emailPlaceholder: 'your@email.com',
        phone: 'Phone',
        phonePlaceholder: '+49 ...',
        service: 'Requested service',
        servicePlaceholder: 'Please select',
        serviceOptions: {
          sprinter: 'Mercedes-Benz Sprinter',
          airport: 'Airport Transfer',
          business: 'Business & Events',
          vip: 'VIP & Chauffeur Service',
          group: 'Group & Bus Transfer',
          private: 'Private Occasions',
          hotel: 'Hotel & Trade Fair Transfer',
          hourly: 'Hourly Booking',
          other: 'Other',
        },
        date: 'Date',
        time: 'Time',
        passengers: 'Number of passengers',
        passengerPlaceholder: 'Please select',
        person: 'Person',
        persons: 'Persons',
        pickup: 'Pick-up location',
        pickupPlaceholder: 'e.g. Düsseldorf Airport, Terminal A',
        destination: 'Destination',
        destinationPlaceholder: 'e.g. Düsseldorf city centre',
        message: 'Special requests',
        messagePlaceholder: 'Child seat, wheelchair access, special wishes...',
        required: '* Required fields. Submitting will open your email client with your pre-filled request.',
        submit: 'Send request by email',
      },
      emailSubject: (service: string, date: string) => `Booking Request – ${service} – ${date || 'Date open'}`,
      emailBody: (d: Record<string, string>) =>
        `Dear VipShuttle24 Team,\n\nI would like to submit a non-binding booking request:\n\n──────────────────────────────\nPERSONAL DETAILS\n──────────────────────────────\nName:         ${d.name}\nEmail:        ${d.email}\nPhone:        ${d.phone}\n\n──────────────────────────────\nRIDE DETAILS\n──────────────────────────────\nService:      ${d.service}\nDate:         ${d.date}\nTime:         ${d.time}\nPassengers:   ${d.passengers}\nPick-up:      ${d.pickup}\nDestination:  ${d.destination}\n\n──────────────────────────────\nFURTHER INFORMATION\n──────────────────────────────\n${d.message}\n──────────────────────────────\n\nPlease contact me as soon as possible.\n\nKind regards,\n${d.name}`,
    },
    footer: {
      tagline: "Germany's trusted VIP chauffeur service for celebrities, executives & international clients. Based in Düsseldorf. Operating across Germany and Europe.",
      navTitle: 'Navigation',
      servicesTitle: 'Services',
      contactTitle: 'Contact',
      copyright: (year: number) => `© ${year} VIPSHUTTLE24. All rights reserved.`,
      impressum: 'Legal Notice',
      datenschutz: 'Privacy Policy',
      agb: 'T&Cs',
      whatsappLabel: 'Open WhatsApp chat',
      fbLabel: 'VipShuttle24 on Facebook',
      igLabel: 'VipShuttle24 on Instagram',
      waLabel: 'Contact VipShuttle24 on WhatsApp',
      phoneLabel: 'Phone: +49 177 2472408',
      emailLabel: 'Email: info@vipshuttle24.de',
      addressLabel: 'Address: Schlesische Str. 104, 40231 Düsseldorf',
      hotline: '24/7 Hotline',
      navItems: [
        { label: 'Services', href: '#services' },
        { label: 'Fleet', href: '#fleet' },
        { label: 'Gallery', href: '#gallery' },
        { label: 'Booking', href: '#booking' },
      ],
      servicesList: [
        'VIP Airport Transfer Germany',
        'Celebrity Chauffeur Service',
        'Corporate Roadshow Germany',
        'Hourly Mercedes Hire',
        'Private VIP Driver Germany',
        'Group Coach Charter Germany',
      ],
    },
    legal: {
      back: '← Back to website',
      impressum: {
        pageTitle: 'Legal Notice | VIPSHUTTLE24 Düsseldorf',
        h1: 'Legal Notice',
        s1Title: 'Information pursuant to § 5 TMG',
        s1Name: 'Ahmet Bostanci',
        s1Company: 'Vipshuttle24',
        s1Street: 'Schlesische Str. 104',
        s1City: '40231 Düsseldorf',
        s2Title: 'Contact',
        s2Mobile: 'Mobile',
        s2Phone: 'Phone',
        s2Email: 'Email',
        s3Title: 'General Disclaimer',
        s3Text:
          'No liability is accepted for completeness, editorial or technical errors, omissions or the accuracy of entries. In particular, no guarantee can be given for the completeness and accuracy of information reached via external links. Providers are responsible for their own content under general law. References via hyperlinks to third-party website content serve purely for information purposes.',
        s4Title: 'Dispute Resolution',
        s4Text: 'The European Commission provides a platform for online dispute resolution (ODR):',
        s4Suffix:
          'We are neither willing nor obliged to participate in dispute resolution proceedings before a consumer arbitration board.',
      },
      datenschutz: {
        pageTitle: 'Privacy Policy | VIPSHUTTLE24 Düsseldorf',
        h1: 'Privacy Policy',
        s1Title: '1. Privacy at a Glance',
        s1h2: 'General Information',
        s1p1:
          'The following information provides a simple overview of what happens to your personal data when you visit this website. Personal data is any data that can be used to identify you personally.',
        s1h3: 'Data Collection on this Website',
        s1p2:
          'Data processing on this website is carried out by the website operator. Your data is collected partly by you providing it to us (e.g. via a contact form). Other data is collected automatically when you visit the website by our IT systems (technical data such as internet browser, operating system or time of page access).',
        s1h4: 'Your Rights',
        s1p3:
          'You have the right at any time to receive free information about the origin, recipient and purpose of your stored personal data, as well as the right to correction, blocking or deletion of this data. You also have the right to lodge a complaint with the competent supervisory authority.',
        s2Title: '2. Hosting',
        s2h2: 'RAIDBOXES',
        s2p1:
          'We host the content of our website with RAIDBOXES GmbH, Hafenstr. 32, 48151 Münster. When you visit our website, RAIDBOXES collects various log files including your IP addresses. RAIDBOXES is used on the basis of Art. 6(1)(f) GDPR.',
        s2h3: 'Cloudflare',
        s2p2:
          'We use the service by Cloudflare Inc., 101 Townsend St., San Francisco, CA 94107, USA. Cloudflare offers a globally distributed content delivery network. Use is based on Art. 6(1)(f) GDPR. The company holds certification under the EU-US Data Privacy Framework (DPF).',
        s3Title: '3. Responsible Party',
        s3Name: 'Vipshuttle24',
        s3Person: 'Ahmet Bostanci',
        s3Street: 'Schlesische Str. 104',
        s3City: '40231 Düsseldorf',
        s3Phone: 'Phone',
        s3Mobile: 'Mobile',
        s3Email: 'Email',
        s4Title: '4. Data Collection on this Website',
        s4h2: 'Cookies',
        s4p1:
          'Our website uses so-called cookies. Cookies are small data packages stored on your device. You can set your browser to inform you when cookies are being set.',
        s4h3: 'Contact Form & Email',
        s4p2:
          'If you send us enquiries via contact form or email, your details including the contact information you provide will be stored by us for the purpose of processing the request. We do not share this data without your consent. Legal basis: Art. 6(1)(b) GDPR.',
        s4h4: 'Communication via WhatsApp',
        s4p3:
          'For communication we use WhatsApp Business by WhatsApp Ireland Limited, 4 Grand Canal Square, Dublin 2, Ireland. Communication is end-to-end encrypted. Legal basis: Art. 6(1)(f) GDPR. The company holds certification under the EU-US Data Privacy Framework.',
        s5Title: '5. Social Media',
        s5p1:
          'This website integrates social media elements (Facebook, Instagram). These are only used together with the Shariff solution, which prevents data from being transmitted when the page loads. A connection is only established when you actively click on the social media element (Art. 6(1)(a) GDPR).',
        s6Title: '6. Analytics Tools',
        s6h2: 'WP Statistics',
        s6p1:
          'This website uses WP Statistics (Veronalabs, Tallinn, Estonia) for anonymised analysis of visitor access. The collected data is stored exclusively on our own server. IP addresses are anonymised. Legal basis: Art. 6(1)(f) GDPR.',
        s7Title: '7. Plugins and Tools',
        s7h2: 'YouTube (Enhanced Privacy)',
        s7p1:
          'This website embeds videos from YouTube (Google Ireland Limited, Dublin 4, Ireland). We use enhanced privacy mode. Legal basis: Art. 6(1)(f) GDPR. Google is certified under the EU-US Data Privacy Framework.',
        s7h3: 'Font Awesome',
        s7p2:
          'This site uses Font Awesome (Fonticons, Inc., Cambridge, Massachusetts, USA) to display icons. Legal basis: Art. 6(1)(f) GDPR.',
        s7h4: 'OpenStreetMap',
        s7p3:
          'We embed OpenStreetMap on our own server, so no connection to third-party servers is established. Legal basis: Art. 6(1)(f) GDPR.',
        s8Title: '8. SSL Encryption',
        s8p1:
          'For security reasons, this site uses SSL or TLS encryption. You can recognise an encrypted connection by the browser address bar changing from http:// to https://.',
      },
      agb: {
        pageTitle: 'T&Cs | VIPSHUTTLE24 Düsseldorf',
        h1: 'Terms & Conditions',
        preise: { title: 'Prices', text: 'All prices are in euros and include statutory VAT of 19%.' },
        zahlung: { title: 'Payment', text: 'Payment for services is possible by bank transfer in advance or by credit card. We do not accept cash payments.' },
        leistung: { title: 'Scope of Services', text: 'Rental prices include driving services, fuel costs and complimentary water in the vehicle. Parking and toll fees are charged separately.' },
        mindest: { title: 'Minimum Hire Duration', text: 'The minimum hire duration for hourly bookings is three hours.' },
        wartezeit: { title: 'Waiting Time', text: '30 minutes waiting time at the airport and 15 minutes at other pick-up locations are included in the price. Additional waiting time at the pick-up location is charged in 30-minute increments.' },
        fahrpreis: { title: 'Fare', text: 'The fare does not change during the journey due to traffic-related changes in the route or waiting times.' },
        rauchen: { title: 'No Smoking', text: 'Smoking is prohibited in all of our vehicles.' },
        haftungKunde: { title: 'Customer Liability', text: 'Customers assume full financial liability for damage caused by themselves or members of their party during the hire/journey period.' },
        haftungsausschluss: { title: 'Disclaimer of Liability', text: 'The company is not responsible for delays due to weather, traffic or unsafe road conditions (natural events, accidents or unforeseeable technical breakdowns). In such cases, the contract may be cancelled without compensation. Our liability is generally limited to the legally required public liability insurance.' },
        storno: {
          title: 'Cancellation Policy',
          row1label: 'Cancellation up to 7 days before departure',
          row1value: 'Free of charge',
          row2label: 'Cancellation up to 2 days before departure',
          row2value: '50%',
          row3label: 'Cancellation up to 24 hours before departure',
          row3value: '75%',
          row4label: 'Cancellation from 24 hours before departure',
          row4value: '100%',
        },
        gericht: { title: 'Jurisdiction', text: 'The exclusive place of jurisdiction is Düsseldorf, insofar as the client is a merchant, a legal entity under public law or a special fund under public law. This also applies if the client has no general place of jurisdiction in Germany or another EU member state.' },
      },
      links: {
        impressum: 'Legal Notice',
        datenschutz: 'Privacy Policy',
        agb: 'T&Cs',
        home: 'Home',
      },
    },
  },
} as const;

export type Translations = typeof translations.de;
